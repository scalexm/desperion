...
namespace network
{
    ...
    {
        std::vector<alliance_fact_sheet_informations_ptr> alliances;
        
        int16_t id() const override
        { return 6408; } 
        
        alliance_list_message(const std::vector<alliance_fact_sheet_informations_ptr> & alliances)
        {
        ...
        alliance_list_message(byte_buffer & data)
        {
            int16_t alliances_size;
            data >> alliances_size;
            alliances.resize(alliances_size);
            for (int16_t i = 0; i < alliances_size; ++i)
                alliances[i] = std::make_unique<alliance_fact_sheet_informations>(data);
        }
    };
}
